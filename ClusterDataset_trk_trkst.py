    ...
    def download(self):
        ...
        for id in range(len(files)):
            ...
            for ev in range(len(tsCP)):
                valid_indices = set()   # dedupe, since a track can now be reached via multiple sim particles
                track_ids = ak.to_numpy(data_trk[ev]["track_id"])   # convert once per event, not per lookup
                n_trk = len(track_ids)
                track_sim_match = [[] for _ in range(n_trk)]

                for sim_idx in range(len(tsCP[ev]["trackIdx"])):
                    for track_id_value in tsCP[ev]["trackIdx"][sim_idx]:
                        matches = np.where(track_ids == track_id_value)[0]
                        if len(matches) == 0:
                            continue
                        track_position = matches[0]
                        valid_indices.add(track_position)
                        track_sim_match[track_position].append(sim_idx)
