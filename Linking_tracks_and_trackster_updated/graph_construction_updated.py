...
#   col 3: dist3D      3D distance between the two positions #remove
#   col 4: distXY      transverse (xy) distance # remove
...
def construct_graphs(event, data_trk, data_trkst, event_nodes,
    ...
    try:
        ...
        # (b) ts-ts edges: single direction only (inner -> outer, as discovered
        # by nodes_construction's z-ordering rule).
        ...
        edge_features[:, 3] = tgt_E - src_E
        edge_features[:, 4] = tgt_E / (src_E + 1e-8)
        ...
        dt_tt = (tgt_time - src_time) - tof
        deltaTime = np.where(is_tt, dt_tt, dt_kt)
            
        edge_features[:, 5] = np.where(valid_time, deltaTime, 0.0)

        terr_comb = np.sqrt(src_terr**2 + tgt_terr**2 + 1e-8)
        edge_features[:, 6] = np.where(valid_time,
                                       (tgt_time - src_time) / terr_comb, 0.0)

        # --- col 9: edge type ------------------------------------------------
        edge_features[:, 7] = etype
        ...
        for e in range(num_edges):
            ...
            if etype[e] == EDGE_TYPE_TS_TS:# if the src is ts this will work
                ...
                y[e] = 1.0 if (src_sims & tgt_sims) else 0.0# here ------------------------------
            else:# if the src is trk this will work
                if have_trk_sim:
                    orig_trk = trk_index_inv[src[e]]
                    y[e] = 1.0 if trk_sim_full[orig_trk] in tgt_sims else 0.0
